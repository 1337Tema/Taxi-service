...
graph TD
    ...
    OrderService --"4. Сохранить заказ (status: pending)"--> PostgreSQL
    OrderService --"5. Публикация события 'OrderCreated'"--> Message_Broker
    Message_Broker -.->"6. Новый заказ"-.-> DriverMatchingService
    DriverMatchingService --"7. Найти ближайших водителей"--> Redis
    DriverMatchingService --"8. Заблокировать водителя"--> Redis
    DriverMatching-.-NotificationService
    DriverMatchingService -.-> "9. Отправить предложение водителю" -.-> NotificationService
    NotificationService --"10. WebSocket Push"--> Driver

    %% --- Сценарий 2: Обновление статуса водителя (Синхронный поток) ---
    Driver --"A. PUT /api/v1/drivers/me/presence"--> API_Gateway
    ...
    %% (Например, водитель принял заказ)
    Driver -- "Принял заказ" --> API_Gateway --> OrderService
    OrderService -- "Обновить статус в БД" --> PostgreSQL
    OrderService -- "Публикация события 'DriverAssigned'" --> Message_Broker
